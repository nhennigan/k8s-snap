...
def extract_commands(file_path):
    ...
    if file_ext in ['.rst', '.rest']:
        return extract_commands_from_rst(file_path)
    if file_ext in ['.md', '.markdown']:
        return extract_commands_from_markdown(file_path)
...
Special Markers:
  ...
  SPREAD SUITE marker (optional routing target):
    ...
    try:
        ...
        # When spread-config is provided, pre-create all suite directories so
        # that spread can list them even if no tasks have been generated yet.
        ...
        if args.spread_config and is_dir_output:
            output_path.mkdir(parents=True, exist_ok=True)
            allowed_suites = get_allowed_spread_suites(args.spread_config)
            if allowed_suites:
                for suite in allowed_suites:
                    # Only create top-level suite dirs (basenames, not full paths)
                    if '/' not in suite:
