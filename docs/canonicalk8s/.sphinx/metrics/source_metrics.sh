...
files=$(find ../src/ -type d -path './.sphinx' -prune -o -type f \( -name '*.md' -o -name '*.rst' \) -print | wc -l)
...
if [ "$files" -eq 0 ]; then
    ...
    words=$(find ../src/ -type d -path './.sphinx' -prune -o \( -name '*.md' -o -name '*.rst' \) -exec cat {} + | wc -w)
